// ESP32/mbedTLS-only — mirrors the same guard used by
// certificate_validator_mbedtls.cpp. native_openevse (host-side tests) swaps
// in CERT_VALIDATOR_OPENSSL and has no use for generating a local HTTPS
// server certificate.
...
// mbedTLS 3.x needs an RNG callback wired through explicitly in several
// places below; back it with the ESP HW RNG (same approach already used in
// certificate_validator_mbedtls.cpp for mbedtls_pk_parse_key()).
...
// Builds and sets the SubjectAltName extension (dNSName + iPAddress) using
// only baseline ASN.1 writer primitives (mbedtls_asn1_write_*), present in
// mbedTLS since well before mbedtls_x509write_crt_set_subject_alternative_name()
// existed (that higher-level helper is mbedTLS 3.5+ only) — this way SAN
// support works identically on both the older mbedTLS bundled with the
// classic ESP32 Arduino core and the newer one bundled with the core-3
// toolchain. Returns 0 on success, a negative mbedTLS error code otherwise
// (mbedTLS convention, not bool, since MBEDTLS_ASN1_CHK_ADD-style early
// returns hand back that error code directly).
static int addSubjectAltNames(mbedtls_x509write_cert &crt, const String &dnsName, const String &ipAddress)
{
  ...
  if(ipAddress.length() > 0 && ip.fromString(ipAddress)) {
    ...
    entryLen += ret;
    if((ret = mbedtls_asn1_write_tag(&p, buf, MBEDTLS_ASN1_CONTEXT_SPECIFIC | 7)) < 0) return ret; // [7] iPAddress
    entryLen += ret;
    len += entryLen;
  }
  ...
  if(dnsName.length() > 0) {
    ...
    entryLen += ret;
    if((ret = mbedtls_asn1_write_tag(&p, buf, MBEDTLS_ASN1_CONTEXT_SPECIFIC | 2)) < 0) return ret; // [2] dNSName
    entryLen += ret;
    len += entryLen;
  }
...
bool generateSelfSignedCertificate(const String &commonName, const String &ipAddress, String &certPem, String &keyPem)
{
  ...
  if(ok) {
    ...
    if(ok) {
      ...
      serialBytes[0] &= 0x7F; // keep the serial positive (X.509 INTEGER encoding)
