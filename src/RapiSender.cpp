...
void RapiSender::_sendCmd(const char *cmdstr)
{ 
  ...
  if (_sequenceIdEnabled()) {
    if (++_sequenceId == RAPI_INVALID_SEQUENCE_ID) ++_sequenceId;
...
//        = 1 = bad checksum
//        = 2 = bad sequence id
int RapiSender::_tokenize()
{
  uint8_t chk = 0;
  char *s = _respBuf;
  dbgprint("resp: ");dbgprintln(_respBuf);
  ...
  while (*s) {
    _tokens[_tokenCnt++] = s++;
    if (_tokenCnt == RAPI_MAX_TOKENS) break;
 ...
 * 3=cmdstr too long
*/
int RapiSender::sendCmd(const char *cmdstr,unsigned long timeout)
{
  _sendCmd(cmdstr);
...
#ifdef DBG
  dbgprint("\n\rTOKENCNT: ");dbgprintln(_tokenCnt);
...
void RapiSender::enableSequenceId(uint8_t tf)
{ 
  if (tf) {
