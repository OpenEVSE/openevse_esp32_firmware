...
@pytest.mark.timeout(240)
def test_corrupt_certificate_record_can_be_replaced_by_upload(tmp_path):
    chain_path, key_path, _, _ = generate_ecdsa_chain(tmp_path)
    certificate_id = "ABCDEF1234567890"
    payload = {
        "id": certificate_id,
        "name": "recovered-certificate",
        "certificate": chain_path.read_text(encoding="ascii"),
        "key": key_path.read_text(encoding="ascii"),
    }
    ...
    runtime.mkdir()
    filesystem = runtime / "epoxyfsdata"
    certificate_directory = filesystem / "certificates"
    certificate_directory.mkdir(parents=True)
    certificate_path = certificate_directory / f"{certificate_id}.json"
    certificate_path.write_text("{", encoding="ascii")
    log_path = tmp_path / "native.log"
    process = None
    http_port = unused_tcp_port()
    ...
    try:
        ...
        assert json.loads(certificate_path.read_text(encoding="ascii"))["id"] == certificate_id
        assert not list(runtime.rglob("*.tmp"))
